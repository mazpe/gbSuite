...
/**
 * Sample implemenation of FBML, where this particular
...
class MiniFacebookImplementation extends FBMLImplementation {
  ...
  public function fb_name($node)
  {
    ...
    $user_info = user_get_info($uid);
	
	return "<a href='/gbSuite/home.php?app=profile&uid=".$uid."'>".$user_info['name']."</a>";
  ...
  /* gbSuite IMPLEMENTATIONS */
  
  public function fb_js($node)
  {
  	return $node->__toString();
  }
    
  public function fb_error($node)
  {		
		$parts = $this->html_parsing('fb_error',$node->get_attributes());
		
		return $parts["header"]."  ". $node->render_children($this) ."". $parts["footer"];
  }

public function fb_message($node)
{
	$parts = $this->html_parsing('fb_message',$node->get_attributes());
	
	return $parts["header"]."  ". $node->render_children($this) ."". $parts["footer"];
}

public function fb_agency_logo($node)
{
	$parts = $this->html_parsing('fb_agency_logo', $node->get_attributes());
		
	$content = $parts["header"]."  ". $node->render_children($this) ."". $parts["footer"];
	  
	$agencyId = $node->attr("agency_id");
	
	$content = str_replace("[IMAGE]", IMAGES_ROOT."agency/$agencyId/$agencyId.gif", $content);	
	  	
	return $content;
}

public  function fb_logo($node)
{	
	$parts = $this->html_parsing('fb_logo', $node->get_attributes());
			
	$content = $parts["header"]."  ". $node->render_children($this) ."". $parts["footer"];
	
	$content = str_replace("[IMAGE]", IMAGES_ROOT."logo.gif", $content);
	
	return $content;
}

public function fb_img($node)
{
	$parts = $this->html_parsing('fb_img', $node->get_attributes());
			
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
	
	$content = str_replace("[IMAGE]", $node->attr("src"), $content);
	
	if($node->attr("width") != "" && $node->attr("height") != "")
		$content = str_replace("[ATTS]", 'width="'.$node->attr("width").'" height="'.$node->attr("height").'" "'.($node->attr('style') ? 'style="'.$node->attr("style").'"' : ''), $content);
	
	return $content;
}
 
public function fb_picture($node)
{	
	$user = $GLOBALS['gbSuiteUser'];
	
	$attributes  = $node->get_attributes();
	
	if(isset($attributes['uid']))
	{
		$user = user_get_info($attributes['uid']);
	}
	
	$parts = $this->html_parsing('fb_picture', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
			
	if(file_exists($_SERVER['PHP_ROOT']."/images/user/".$user["pic"]) && !is_dir($_SERVER['PHP_ROOT']."/images/user/".$user["pic"]))
	{
		$content = str_replace("[IMAGE]", "/images/user/".$user["pic"], $content); 
	}
	else
	{
		$content = str_replace("[IMAGE]", "/images/user/default_user.gif", $content);
	}
...
public function fb_menu_item($node)
{
	$parts = $this->html_parsing('fb_menu_item', $node->get_attributes());
...
public function fb_menu($node)
{
	$parts = $this->html_parsing('fb_menu', $node->get_attributes());
...
public function fb_data_employee($node)
{
	...
	$parts = $this->html_parsing('fb_data_employee', $attributes);
	
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
	
	$content = str_replace("[ADDRESS]", $user['address'], $content);
	$content = str_replace("[PHONE]", $user['phone'], $content);
	$content = str_replace("[UID]", $user['uid'], $content);
	
	return $content;
}

public function fb_data_personal($node)
{
	$attributes = $node->get_attributes();
	...
	$parts = $this->html_parsing('fb_data_personal', $attributes);
	
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
	
	$content = str_replace("[NAME]", $user['name'], $content);
	$content = str_replace("[STATUS]", $user['status'], $content);
	$content = str_replace("[UID]", $user['uid'], $content);
	
	return $content;
}

public function fb_report($node)
{
	$parts = $this->html_parsing('fb_report', $node->get_attributes());
...
public function fb_report_section($node)
{
	$parts = $this->html_parsing('fb_report_section', $node->get_attributes());
...
public function fb_friend($node)
{
	$parts = $this->html_parsing('fb_friend', $node->get_attributes());
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
	...
	$content = str_replace("[NAME]", $node->attr("name"), $content);
	$content = str_replace("[LINK]", "/gbSuite/home.php?app=profile&uid=".$uid, $content);
		
	$image = $_SERVER['PHP_ROOT']."/images/user/".$user_info['pic'];
	...
	/*if(file_exists($image) && !is_dir($image))	
		$content = str_replace("[IMAGE]", IMAGES_ROOT."user/".$user_info['pic'], $content);
	else
		$content = str_replace("[IMAGE]", IMAGES_ROOT."user/default_user.gif", $content);*/
	
	$title = $user_info["title"];
	$title = strtolower($title);
	$title = str_replace(" ", "", $title);
	
	$content = str_replace("[IMAGE]", IMAGES_ROOT."user/$title.gif", $content);
	
	//$content = str_replace("[IMAGE]", IMAGES_ROOT."user/$sex"."_associate.gif", $content);
	
	//if($node->attr("width") != "" && $node->attr("height") != "")
	//	$content = str_replace("[ATTS]", "width=".$node->attr("width")." height=".$node->attr("height"), $content);
	
	return $content;	
}

public function fb_friend_section($node)
{
	$parts = $this->html_parsing('fb_friend_section', $node->get_attributes());
...
public function fb_associate($node)
{
	$parts = $this->html_parsing('fb_associate', $node->get_attributes());
	...
	$content = str_replace("[LINK]", "/gbSuite/home.php?app=profile&uid=".$fuid, $content);
	
	$content = str_replace("[UID]", $uid, $content);
	$content = str_replace("[FUID]", $fuid, $content);
	
	if($node->attr("added") == "0")
	{
		$content = str_replace("[LINK_ADD_REMOVE_FRIEND]", "a1234567_addFriend('".$fuid."')", $content);
		$content = str_replace("[ADD_REMOVE_TEXT]", "Add to my associates", $content);
		$content = str_replace("[FUID]", $fuid, $content);
			
		//$content = str_replace("[LINK_ADD_FRIEND]", "/gbSuite/home.php?app=associates&action=add&uid=".$fuid."&ui=false", $content);
		//$content = str_replace("[DISPLAY_ADD_FRIEND]", "block", $content);	
	}
	else		
		if($node->attr("added") == "1" )
		{
			$content = str_replace("[LINK_ADD_REMOVE_FRIEND]", "a1234567_removeFriend('".$fuid."')", $content);
			//$content = str_replace("[LINK_ADD_REMOVE_FRIEND]", "a1234567_removeFriend('".$fuid."')", $content);
			$content = str_replace("[ADD_REMOVE_TEXT]", "Remove from my associates", $content);
			$content = str_replace("[FUID]", $fuid, $content);
				
			//$content = str_replace("[LINK_ADD_FRIEND]", "#", $content);	
			//$content = str_replace("[DISPLAY_ADD_FRIEND]", "none", $content);
		}
	
	if($node->attr('user_profile') == 1)
		$content = str_replace("[DISPLAY_ADD_REMOVE_FRIEND]", "block", $content);
	else
		if($node->attr('me') == "0" && $node->attr("added") == "0")
			$content = str_replace("[DISPLAY_ADD_REMOVE_FRIEND]", "block", $content);				
		else
			$content = str_replace("[DISPLAY_ADD_REMOVE_FRIEND]", 'none', $content);
					
	if($node->attr("title") == "Administrator")
	{		
		$content = str_replace("[LINK_EDIT]", "/gbSuite/home.php?app=profile&action=edit&uid=".$fuid, $content);
		$content = str_replace("[LINK_DELETE]", "/gbSuite/home.php?app=profile&action=delete&uid=".$uid."&fuid=".$fuid."&ui=false", $content);
		
		$content = str_replace("[DISPLAY]", "block", $content);
	}
	else
	{
		$content = str_replace("[DISPLAY]", "none", $content);
		
		$content = str_replace("[LINK_EDIT]", "#", $content);
		$content = str_replace("[LINK_DELETE]", "#", $content);
	}
	
	$user_info = user_get_info($fuid);
...
public function fb_associate_section($node)
{
	$parts = $this->html_parsing('fb_associate_section', $node->get_attributes());
...
public function fb_left_column($node)
{
	$parts = $this->html_parsing('fb_left_column', $node->get_attributes());
...
public function fb_editor_text($node)
{
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_editor_text', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
	
	 $delete = array_diff(array("label","maxlength","value","name"),array_keys($attributes));
	
	foreach($delete as $att)
	{
...
public function fb_editor_textarea($node)
{
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_editor_textarea', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
	
	 $delete = array_diff(array("label","name"),array_keys($attributes));
...
public function fb_editor_label($node)
{
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_editor_label', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
...
public function fb_editor_check($node)
{
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_editor_check', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
...
public function fb_login($node)
{
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_login', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
		
	 $delete = array_diff(array("title","name"),array_keys($attributes));
...
public function fb_header_bar($node)
{
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_header_bar', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
...
public function fb_links($node)
{
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_links', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
...
public function fb_profile_links($node)
{
	$attributes = $node->get_attributes();

	$ui_id = $attributes['uid'];

	$parts = $this->html_parsing('fb_profile_links', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
	...
	return fbml_sample_parse($content,$this);
} 

public function fb_title($node)
{
	$parts = $this->html_parsing('fb_title', $node->get_attributes());
	
	$content = $parts["header"]."  ".$node->render_html($this) ."". $parts["footer"];
...
public function fb_li($node)
{
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_li', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
...
public function fb_footer_bar($node)
{
	//return "";
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_footer_bar', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
...
public function fb_news_box($node)
{ 
	global $data_conn;
	
	$attributes = $node->get_attributes();
	$parts = $this->html_parsing('fb_news_box', $attributes);
	
	
	$parts['header'] =  str_replace("[title]","",$parts['header']);
		
	$uid = $attributes["uid"];
	
	$sql = "select uid,type,value,date,
			if(date(date) = date(now()),'Today', 
			if(adddate(date(date),INTERVAL 1 DAY) = date(now()),'Yesterday',DATE_FORMAT(date, '%M %d'))
			) as day
			from news where uid='$uid' order by date DESC
			limit 5 
			";
	...
	$cont_news = mysql_num_rows($result);
	
	if(!$cont_news)
	{
		$body .= "<fb:day-news title=''/>";
		$body .= "<fb:day-news title=''>";
		$body.= "<fb:news-item type='no_news'> No news to display. </fb:news-item>";
	}
	else
	{
		while($row = mysql_fetch_assoc($result))
		{
			...
			if($cont == 0)
			{
				$body .= "<fb:day-news title='" . $row['day'] .  "'>";
				$body.= "<fb:news-item type='". $row['type'] ."'> ". $row['value'] ." </fb:news-item>";
			}
			else
			{
				$body.= "<fb:news-item type='". $row['type'] ."'> ". $row['value'] ." </fb:news-item>";
	...
	$content = $parts["header"]. fbml_sample_parse($body,$this) ."  ".$node->render_children($this) ."". $parts["footer"];
	
	return $content;
} 
	
public function fb_day_news($node)
{
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_day_news', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];

	return $content;
} 

public function fb_news_item($node)
{
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_news_item', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];

	$image = IMAGES_ROOT."resources/".$attributes["type"].".gif";
...
public function fb_config_table($node)
{
	$attributes = $node->get_attributes();
	
	$parts = $this->html_parsing('fb_config_table', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];
	...
	if($node->attr('buttons') == true)
		$content = str_replace("[BUTTONS]", "", $content);
	else
		if($node->attr('buttons') == false)
			$content = str_replace("[BUTTONS]", "<a href='/gbSuite/home.php?app=applications&action=view_more' class='button-link'  width=100%>View&nbsp;More&nbsp;</a>&nbsp;
												 <a href='/gbSuite/home.php?app=applications&action=order' class='button-link' width=100%>Change&nbsp;Order</a>", $content);	
			
	return $content;
} 

public function fb_config_table_item($node)
{
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_config_table_item', $attributes);
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];

	return $content;
}

public function fb_application_title($node)
{
	$attributes = $node->get_attributes();
			
	$parts = $this->html_parsing('fb_application_title', $attributes);
	
	$content = $parts["header"]."  ".$node->render_children($this) ."". $parts["footer"];

	$content = str_replace("[NAME]", $node->attr('name'), $content);
	$content = str_replace("[ID]", $node->attr('id'), $content);
...
/**
 * This function takes the html pre-configured code for an fbml tag
 * and puts it into cache and returns its values 
 *  
 * If the fbml tag is already in cache we just return its 
 * value.
 */
public function html_parsing($file_name, $attrib)
{
	global $fbml_cache;
		
	if(!isset($fbml_cache[$file_name]))
	{
		$file_url= TEMPLATES_ROOT.$file_name.TEMPLATES_FILE_EXT;
		$html=file_get_contents($file_url);
		$part= explode('[BODY]', $html);
		$array=array('header'=>$part[0],'footer'=>$part[1]);
		
		$atts = "";
		$fbml_cache[$file_name]=$array;
		
		foreach($attrib as $att => $value)
		{
			$array['header'] = str_replace("[$att]",$value,$array['header']);
