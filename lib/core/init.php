 ...
 */
 ...
 $post_require_login = false;

if(!extension_loaded('fbml'))
{
        dl('fbml.' . PHP_SHLIB_SUFFIX);
}

function get_site_variable($vari)
{
	return 2;
}

define("IS_DEV_SITE", false );
define("IS_PRODUCTION_ANY", true );
...
$DB_USERNAME = $db_user; // FBOPEN:NOTE - replace here with your own.
$DB_IP = $db_ip; // FBOPEN:NOTE - replace here with your own.
$DB_PWD = $db_pass; // FBOPEN:NOTE - replace here with your own.

$data_conn = mysql_connect($DB_IP, $DB_USERNAME, $DB_PWD);
$data_conn2 = $data_conn;
$select_result = mysql_select_db($db_name, $data_conn);
...
$PLATFORM_JS_FILES = array(
   ...
   function fbml_flavors_get_codes(){
    global $FBML_FLAVORS;
    return $FBML_FLAVORS;
  }