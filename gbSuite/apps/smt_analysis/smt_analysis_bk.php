<?php
    ...
    class SMTAnalysis extends Application
	{
		...
		private function renderTable()
		{
			...
			$query = "SELECT IFNULL(team, 'Other') AS team, name, I.uid, SUM(`units`) AS `units`, IF(G.`recommit` > 0, G.recommit, G.goals) AS `goal`, SUM(IF(units > 0, units / $currentDay, 0) * $monthDayCount) AS `track`,SUM(`new`) AS `new`,SUM(`used`) AS `used`, SUM(`gross_front`) AS `gross_front`,SUM(`gross_back`) AS `gross_back`,
