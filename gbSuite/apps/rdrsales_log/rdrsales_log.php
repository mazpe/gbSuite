<?php
    ...
    class RDRSalesLog extends Application
	{
		var $fields;
		var $printHtml = false;
		...
		public function renderHTML()
		{
			...
			if($this->printHtml == false)
			{
				...
				$sql = "SELECT COUNT(id) FROM app_rpt_desk_log WHERE dl_date BETWEEN '$start' AND '$end' AND certified = 'Y' AND status <> 'B'";
				...
				$sql = "SELECT COUNT(id) AS countTotal FROM app_rpt_desk_log WHERE dl_date BETWEEN '$start' AND '$end' AND status <> 'B'";
				...
				if($this->printing == false)
				{
					...
					$selectionBar = 
						'<form method=post id="desklog-options-form'.$this->appId.'" action="/gbSuite/home.php?app=rdrsales_log">
							<div id="filter" class="" style="text-align:center;">
								...
								<input type=button name="option" class="desklog-dealer-button" value="Today" onclick=document.setLocation("/gbSuite/home.php?app=rdrsales_log") />&nbsp
					...
					$html .= '<form method=post action="/gbSuite/apps/process_application.php?uid='.$this->user->getUID().'&action=set_rdr&redirect=rdrsales_log&'.$processLink.'">';
					...
					$html .= "<tr>";
						...
						if($this->printing == false)
							$html .= "<th colspan=2 class='desk-log-header'>Actions</th><th class='desk-log-header'>RDR</th>";
						...
						while($row = mysql_fetch_array($resuts))
						{
							...
							if($this->printing == false)
							{
								...
								$html .= "<td class='desk-log-cell'><a href='/gbSuite/home.php?app=rdrsales_log&uid".$this->user->getUID()."&action=delete&id=".$row['id']."&confirm=true'><img src='/images/resources/delete.gif' /></a></td>";	
								$html .= "<td class='desk-log-cell' align=center>
												<select name=rdrsales_log_".$row['id']." class=desk-log-models>
		...
		public function delete($params)
		{
			...
			$powerRankSetup['total_percentage'] = ($powerRankSetup['total_percentage'] > 0 ? $powerRankSetup['total_percentage'] / 100 : 0);
				...
				if($params['confirm'] == 'true')
				{
					...
					$html .= '<div><a class="button-link" href="/gbSuite/apps/process_application.php?app=rdrsales_log&uid='.$this->user->getUID().'&action=delete&id='.$params['id'].'&redirect=rdrsales_log" >Yes</a>&nbsp;<a class="button-link" href="/gbSuite/home.php?app=rdrsales_log">Go back</a></div>';
				}
		...
		public function edit($params)
		{
			...
			///gbSuite/apps/process_application.php?app=rdrsales_log&redirect=rdrsales_log&action=save&uid=<?= $this->user->getUID()			
			?>
			<script>
			...
			<form method=post action='/gbSuite/apps/process_application.php?<? echo $processLink."&redirect=rdrsales_log&action=save&uid=".$this->user->getUID(); ?>' id='desk-log'>
			<table width=100% border=0 cellmargin=0 cellpadding=0 cellspacing=0 class='desk-log-table'>
				...
				<?
					...
					//echo "<input class='button-link' type='button' value='Go back' onclick=document.setLocation('/gbSuite/home.php?app=rdrsales_log') />";
