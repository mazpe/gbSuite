<?php
    ...
    class SMPerformance extends Application
	{
		...
		public function renderHTML()
		{
			...
			$title = "Sales Performance by Manager";
			...
			$selectionBar = 
						'<form method=post id="desklog-options-form'.$this->appId.'" action="/gbSuite/home.php?app=sm_performance">
			...
			if(!$this->printing)
			{
				...
				$link = "/gbSuite/apps/process_application.php?app=sm_performance&action=print_report&uid=".$this->user->getUID();
		...
		private function renderTable()
		{
			...
			$fields = array("`team`", "`units`", "`goal`", "`track`", "`new`", "`used`", "`gross_front`", "`gross_back`", "`gross_total`", "`mtd_avg`", "`showroom`", "`sold_sh`", "`close_sh%`", "iphone", "sold_ip", "`close_ip%`", "ileads", "sold_il", "`close_il%`");
			$totalType = array("none", "sum", "sum", "sum", "sum", "sum", "sum", "sum", "sum", "mtdavg", "sum", "sum", "avg", "sum", "sum", "avg", "sum", "sum", "avg");			
			$header = array("Team", "Units", "Goal", "Track", "N", "U", "Front", "Back", "Total", "MTD AVG", "Show Room", "Sold", "%", "iPhone", "Sold", "%", "iLead", "Sold", "%");
			...
			$query = "SELECT team, SUM(`units`) AS `units`, SUM(`goal`) AS goal, SUM(`track`) AS `track`,SUM(`new`) AS `new`,SUM(`used`) AS `used`, SUM(`gross_front`) AS `gross_front`,SUM(`gross_back`) AS `gross_back`,
						...
						WHERE D.id = $this->dealerViewId AND I.title = 'Salesperson' AND I.employee_id IS NOT NULL AND I.active
						GROUP BY team, name ORDER BY team) T
						GROUP BY team ORDER BY `$this->sort` $this->order";
			...
			$link = '/gbSuite/home.php?app=sm_performance&order='.$order2;
			...
			while($row = mysql_fetch_array($rsTeam, MYSQL_ASSOC))
			{
				...
				for($i = 0; $i < count($fields); $i++)	
				{
					...
					if($field != 'mtd_avg')
					{
						...
						$html .= '<td onMouseover=setClass("report-cell-over",this) onMouseout=setClass("report-cell",this) align="'.$align[$i].'"><a href="/gbSuite/home.php?app=sp_analysis&suid='.$row['uid'].'">'.$this->formatNumber($value, $format[$i])."</a></td>";						
					}											
					else
						$html .= '<td onMouseover=setClass("report-cell-over",this) onMouseout=setClass("report-cell",this) align="'.$align[$i].'"><a href="/gbSuite/home.php?app=sp_analysis&suid='.$row['uid'].'">'.$this->formatNumber(($row['units'] > 0 ? $row['gross_total'] / $row['units'] : 0), $format[$i])."</a></td>";						
