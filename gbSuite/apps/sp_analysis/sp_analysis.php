<?php
    ...
    class SPAnalysis extends Application
	{
		...
		public function renderHTML()
		{
			...
			if(isset($_GET['suid']))
				...
				$this->uid = $this->user->getCurrentProfileUID();
			...
			$this->userTitle = $this->user->getTitle(); //Title of the logged user
			{
				...
				$selectionBar = 
						'<form method=post id="desklog-options-form'.$this->appId.'" action="/gbSuite/home.php?app=sp_analysis&uid='.$this->user->getUID().'&suid='.$uid.'">
				...
				if(!$this->printing)
				{
					...
					$link = "/gbSuite/apps/process_application.php?app=sp_analysis&action=print_report&uid=".$this->user->getUID()."&suid=".$uid;
		...
		private function renderTable()
		{
			...
			$totalType = array("none", "sum", "sum", "sum", "sum", "sum", "sum", "sum", "sum", "mtdavg", "sum", "sum", "avg", "sum", "sum", "avg", "sum", "sum", "avg");			
			$header = array("Associate", "Units", "Goal", "Track", "N", "U", "Front", "Back", "Total", "MTD<br/>AVG", "Show<br/>Room", "Sold", "%", "iPhone", "Sold", "%", "iLead", "Sold", "%");
			...
			/*$query =	"SELECT `sales_person_1`,SUM(`units`) AS `units`,SUM(`goal`) AS `goal`,SUM(`track`) AS `track`,SUM(`new`) AS `new`,SUM(`used`) AS `used`,SUM(`gross_front`) AS `gross_front`,SUM(`gross_back`) AS `gross_back`,SUM(`gross_total`) AS `gross_total`,SUM(`mtd_avg`) AS `mtd_avg`,
							...
							L.ileads, SUM(`sold_il`) AS `sold_il`, IF(L.ileads > 0, SUM((`sold_il` * 100) / L.ileads), 0) AS `close_il%`, I.uid 
						FROM app_rpt_sales_department R INNER JOIN info I 
							ON R.employee_id = I.employee_id INNER JOIN lead_control L 
							ON L.uid = I.uid LEFT JOIN goals_settings G 
								O
						$this->whereClause GROUP BY sales_person_1";*/
						
			$query = "SELECT name, I.uid, SUM(`units`) AS `units`,IF(G.`recommit` > 0, G.recommit, G.goals) AS `goal`, SUM(`track`) AS `track`,SUM(`new`) AS `new`,SUM(`used`) AS `used`, SUM(`gross_front`) AS `gross_front`,SUM(`gross_back`) AS `gross_back`,
						...
						WHERE I.uid = '$this->uid' AND D.id = $this->dealerViewId AND I.title = 'Salesperson' AND I.employee_id IS NOT NULL and I.active = 1
						GROUP BY name";
			...
			for($i = 0; $i < count($fields); $i++)	
			{
				...
				//$html .= '<td style="color:blue;" align="'.(($totalType[$i] == "none")?"left" : "right").'">';
				$html .= '<td style="color:blue;" align="'.$align[$i].'">';
				
				if($totalType[$i] == "sum")					
					//$html .= number_format($groupSum[$field], 2 , "." , ",");
					...
					if($rowCount > 0)
					{
						if($totalType[$i] == "avg")
						{
							...
							//$html .= $this->formatNumber(($groupSum[$field]/$groupRowCount), $format[$i]);
						}	
						else
							if($totalType[$i] == "mtdavg")
							{
								if($sum["units"] > 0)
									$html .= $this->formatNumber(($sum["gross_total"]/$sum["units"]), $format[$i]);
								else
