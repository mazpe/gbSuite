...
if (!($user = $get_fb_user_id) && $url_suffix != 'login_data.php') {
  print('No user id parameter');
  error_log('No user id parameter');
  exit();
}

$canvas_url = redirect_str($rel_canvas_url, '',
...
if ($use_iframe) {
  ...
  $post_vars['app_id'] = $app_id;
  $post_vars['app_secret'] = $app_info['secret'];
  $post_vars['api_key'] = $app_info['apikey'];
  $post_vars['me_uid'] = $_SESSION['uid'];
  $post_vars['SESSION'] = $_SESSION;
...
//echo '<div id="' . fbml_css_app_box_id($app_id) . '" class="'.fbml_css_app_box_id($app_id).' ">';
echo $html;
//echo '</div>'; 
